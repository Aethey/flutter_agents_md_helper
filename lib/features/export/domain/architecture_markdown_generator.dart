...
class ArchitectureMarkdownGenerator {
  ...
  List<String> _conventions(
    ...
    if (ids.contains('gen-l10n')) {
      lines.add(
        _pick(
          languageCode,
          'Own strings live in ARB files; do not hand-edit generated localizations.',
          '自有文案放在 ARB 中；不要手改生成的本地化代码。',
          'アプリ文言は ARB に置き、生成ローカライズは手で編集しない。',
        ),
      );
    }
    if (lines.isEmpty) {
  ...
  List<String> _generatedCodeRules(
    ...
    final usesGenL10n = _optionIds(state).contains('gen-l10n');
    if (!usesCodegen && !usesGenL10n) {
      return [
        _pick(
          languageCode,
    ...
    if (usesGenL10n) {
      lines.add(
        _pick(
          languageCode,
          'Never hand-edit files generated by `flutter gen-l10n`.',
          '不要手改 `flutter gen-l10n` 生成的文件。',
          '`flutter gen-l10n` が生成したファイルは手で編集しない。',
        ),
      );
    }
    return lines;
